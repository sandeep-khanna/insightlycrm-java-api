package com.insightly.api.util;
...
import com.insightly.api.auth.ApiKeyAuth;
import com.insightly.api.auth.Authentication;
import com.insightly.api.auth.HttpBasicAuth;
import com.insightly.api.auth.OAuth;
...
public class ApiClient {
	private static final String VERSION_AGNOSTIC_BASE_PATH = "https://api.insight.ly";
	private static final String VERSION_2_1_BASE_PATH = "https://api.insight.ly/v2.1";
	private static final String VERSION_2_2_BASE_PATH = "https://api.insight.ly/v2.2";
	...
	@Autowired
	public ApiClient(RestTemplate restTemplate, @Qualifier("InsightlyCrmEtl") AppConfig appConfig) {
...
//		authentications.put("ApiKey", apiKeyAuthentication);
	...
	public <T> T invokeAPI(String path, HttpMethod method, MultiValueMap<String, String> queryParams, Object body,
		...
		if(!path.matches("^/v2.*$")) {
			builder = UriComponentsBuilder.fromHttpUrl(VERSION_2_2_BASE_PATH).path(path);
		} else {
			// Build the URI using a version agnostic base path since the version will be provided in the actual 'path' parameter
			builder = UriComponentsBuilder.fromHttpUrl(VERSION_AGNOSTIC_BASE_PATH).path(path);
		}
	...
	private <T> ResponseEntity<T> verifyResponse(ResponseEntity<T> responseEntity, ParameterizedTypeReference<T> returnType) throws RestClientException{
